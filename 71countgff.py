...
'''
Sorted:
Sorted is a function that takes something and sorts it. A list, a dictionary, etc...
...
key= says, give me a function to apply to each item and I will, then I will sort those returns.
In this case key=lambda item:item [1] says, take the lamba function(item) that returns item[1],
...
sorted() returns the dicitionary sorted by values, which can then be iterated through.
'''
